...
{
    /// <summary>
    /// Bunny's F — Jade Hare (ultimate): two-stage ability.
    ...
    /// Bunny is locked in place (VX=VZ=0) during the entire ability.
    /// </summary>
    public class BunnyJadeHare : ServerAbility
    {
        ...
        public override void Tick(ref CharacterState s, ref InputState input, CharacterDefinition def)
        {
            ...
            if (spinElapsed % pullInterval == 0 && SimulationStates != null)
            {
                foreach (var kvp in SimulationStates)
                {
                    ...
                    if (dist <= pullRadius)
                    {
                        // Pull toward Bunny (calculate direction from NPC to Bunny)
