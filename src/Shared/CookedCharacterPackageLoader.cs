    ...
    private static class RuntimeParser
    {
        ...
        private static CookedCapabilityParameters ParseParameters(string id, JsonElement e)
        {
            return id switch
            {
                ...
                "slop.internal.manki.jetpack-boost.v1" => MankiJetpackBoost(O(e, "startupTicks", "verticalSpeed", "horizontalSpeed", "explosionRadius", "explosionDamage", "explosionKbAngle", "explosionKbBase", "explosionKbGrowth", "explosionStunTicks", "explosionDurationTicks")),
        ...
        private static Dictionary<string,JsonElement> O(JsonElement e,params string[] f){if(e.ValueKind!=JsonValueKind.Object)throw new InvalidDataException("Object required.");var set=new HashSet<string>(f,StringComparer.Ordinal);var d=new Dictionary<string,JsonElement>();foreach(var p in e.EnumerateObject())if(!set.Contains(p.Name)||!d.TryAdd(p.Name,p.Value))throw new InvalidDataException("Unknown or duplicate field.");foreach(var x in f)if(!d.ContainsKey(x))throw new InvalidDataException("Missing field.");return d;}
        private static Dictionary<string,JsonElement> OOptional(JsonElement e,string[] optional,params string[] f){if(e.ValueKind!=JsonValueKind.Object)throw new InvalidDataException("Object required.");var set=new HashSet<string>(f,StringComparer.Ordinal);var d=new Dictionary<string,JsonElement>();foreach(var p in e.EnumerateObject())if(!set.Contains(p.Name)||!d.TryAdd(p.Name,p.Value))throw new InvalidDataException("Unknown or duplicate field.");foreach(var x in f)if(!optional.Contains(x,StringComparer.Ordinal)&&!d.ContainsKey(x))throw new InvalidDataException("Missing field.");return d;}
