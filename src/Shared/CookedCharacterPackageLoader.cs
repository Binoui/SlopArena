...
public static class CookedCharacterPackageLoader
{
    ...
    private static bool SupportedCapability(string id)=>id=="slop.internal.fightguy.cyclone-kick.v1"||id=="slop.internal.fightguy.dragon-beam.v1"||id=="slop.internal.fightguy.ki-shot.v1"||id=="slop.internal.fightguy.rising-dragon.v1"||id=="slop.internal.kistu.dash-slash.v1"||id=="slop.internal.kistu.rising-slash.v1"||id=="slop.internal.kistu.blade-flurry.v1"||id=="slop.internal.bonk.targeted-jump-slam.v1";
    ...
    private static class RuntimeParser
    {
        ...
        private static CookedSlotDefinition ParseSlot(JsonElement e){var q=OOptional(e,"aimMovement","ordinal","id","isAir","name","description","iconId","behavior","aimMode","aimMovement","cooldownTicks","isRecoveryMove","preserveMomentumOnStart","chargePool","timeline");var pool=q["chargePool"].ValueKind==JsonValueKind.Null?null:ParseChargePool(q["chargePool"]);var t=O(q["timeline"],"stages");return new CookedSlotDefinition(I(q,"ordinal"),S(q,"id"),Bo(q,"isAir"),S(q,"name"),S(q,"description"),S(q,"iconId"),(AuthoringAbilityBehavior)B(q,"behavior"),(AuthoringAimMode)B(q,"aimMode"),U(q,"cooldownTicks"),Bo(q,"isRecoveryMove"),Bo(q,"preserveMomentumOnStart"),new CookedTimeline(A(t,"stages").EnumerateArray().Select(ParseStage).ToList()),pool,(AuthoringAimMovementMode)BOrDefault(q,"aimMovement",0));}
