...
public static class CookedCharacterPackageLoader
{
    ...
    private static class RuntimeParser
    {
        ...
        private static CookedSlotDefinition ParseSlot(JsonElement e){var q=O(e,"ordinal","id","isAir","name","description","iconId","behavior","aimMode","cooldownTicks","isRecoveryMove","preserveMomentumOnStart","chargePool","timeline");var pool=q["chargePool"].ValueKind==JsonValueKind.Null?null:ParseChargePool(q["chargePool"]);var t=O(q["timeline"],"stages");return new CookedSlotDefinition(I(q,"ordinal"),S(q,"id"),Bo(q,"isAir"),S(q,"name"),S(q,"description"),S(q,"iconId"),(AuthoringAbilityBehavior)B(q,"behavior"),(AuthoringAimMode)B(q,"aimMode"),U(q,"cooldownTicks"),Bo(q,"isRecoveryMove"),Bo(q,"preserveMomentumOnStart"),new CookedTimeline(A(t,"stages").EnumerateArray().Select(ParseStage).ToList()),pool);}
