...
/// <summary>
/// Types de projectiles avec leurs propriétés visuelles et de hitbox.
/// </summary>
public enum ProjectileType
{
	Fireball,    // Boule de feu qui avance vite, petite hitbox
	IceShard,    // Tranchant gelé, étroit et rapide
	ArcaneBlast, // Onde large qui s'élargit
	EarthSpike,  // Pic qui monte du sol
	HolyLight,   // Onde circulaire qui s'étend
}

/// <summary>
/// Un projectile actif dans le monde, avec hitbox mobile et durée de vie.
/// </summary>
public partial class Projectile : Node3D
{
	...
	public bool IsAoE; // Si true, explose/impacte à la fin
	public bool HasHit; // A déjà touché quelque chose
	
	private MeshInstance3D? _mesh;
	private float _distTraveled;
	
	/// <summary>
	/// Crée un projectile à partir d'un sort, visant vers une position cible (réticule).
	/// </summary>
	public static Projectile Create(SpellData spell, ProjectileType type, float startX, float startY, float startZ, float aimX, float aimY)
	{
		...
		proj.PosY = startY;
		proj.PosZ = startZ;
		proj.Radius = 4f; // Valeur par défaut
		proj.Damage = 400f; // Valeur par défaut
		proj.HitstunTicks = 25;
		proj.ArcCos = MathF.Cos(30f * MathF.PI / 180f);
		proj.Range = 60f;
		
		// Direction du joueur vers le réticule
		...
		// Durée de vie et vélocité selon le type
		switch (type)
		{
	...
	/// <summary>
	/// Met à jour la position et la hitbox. Retourne true si encore actif.
	/// </summary>
	public bool Update(float dt, out float hitX, out float hitY, out float hitRadius)
	{
		...
		// 1. Vérifier si on touche le sol (heightmap)
		float groundZ = PhysicsConfig.GetGroundHeight(PosX, PosY);
		if (PosZ <= groundZ + 1f)
		{
			// Le projectile touche le sol - le faire disparaître
			return false;
		}
		
		// 2. Vérifier les limites de l'arène (murs extérieurs)
		float arenaSize = 3000f; // Même taille que les murs CSG
		float halfArena = arenaSize * 0.5f;
		if (MathF.Abs(PosX - 2500f) > halfArena || MathF.Abs(PosY - 2500f) > halfArena)
		{
			return false; // Sorti de l'arène
		}
		
		Lifetime -= dt;
		if (Lifetime <= 0) return false;
		
		// Gravité sur le projectile (sauf Earth Spike et Holy Light)
		if (Type != ProjectileType.EarthSpike && Type != ProjectileType.HolyLight)
		{
			PosZ += VelZ * dt;
			VelZ -= 200f * dt; // Légère gravité
		}
		
		// Mise à jour du rayon pour certains sorts
		switch (Type)
		{
			case ProjectileType.ArcaneBlast:
