...
2. **The lock is PASSIVE (amended 2026-08-17, implemented 3b3d9db1)** — it keeps the resolved target fresh for attack auto-face, camera framing, and the lock indicator, but does **not** steer facing while moving: the fighter runs where it runs (sticky air facing intact), and only turns toward the target during attacks (per-stage `RotateTowardTarget`). The original active-tracking design (facing continuously lerped to the target, ground + air) was rejected during implementation — constant facing override read as robotic and deleted the spacing/back-air game entirely; passive keeps the casual auto-aim (attacks still auto-face) without removing movement facing reads. Turn rate is therefore per-stage `TrackingStrength`, not a global lock lerp.
