...
SlopArena uses a **sim-authoritative** hitbox system driven by **HitboxEvent** data. Hitboxes are spawned by `ServerSimulation.Tick()` when the elapsed attack time matches a `HitboxEvent.TriggerTick`, using pure math collision detection (no engine physics queries on the server).
...
```
SlopArena/
├── Shared/                          ← Pure C#, no engine dependency
...
├── Scripts/
│   ├── Tools/
│   │   └── SlopArenaBaker.cs       ← Unity Editor tool: bakes bone positions per anim frame → .bin
│   ├── Combat/
...
│
├── data/
│   └── manki_skeleton.bin           ← Pre-baked bone positions (generated by SlopArenaBaker)
│
└── docs/
...
GLB (with skeleton + animations)
        │
        ▼
  SlopArenaBaker.cs           ← Run once in the Unity Editor (Assets/Scripts/Editor/SlopArenaBaker.cs)
        │
  ...
  data/manki_skeleton.bin       ← 170KB, embed in project
        │
        ▼
  BakedAnimationData.cs         ← Pure C# loader (no engine dependency)
...
- **Pure math**: no engine physics queries (netcode-ready)
...
||| **Bake tool** | ✅ `SlopArenaBaker.cs` — run in Unity Editor to generate .bin |
...
6. **`Assets/Scripts/Editor/SlopArenaBaker.cs`**: Change which bones are baked, add new characters
