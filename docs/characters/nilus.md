...
**Why the rift cannot live inside the ability:** `ServerSimulation.cs:143` discards an ability instance the moment `state.State != ActionState.Attacking`. The old pull implementation's per-tick `SimulationStates` loop only works because it locks the caster in `Attacking` for its whole duration. A placed rift must outlive the cast, so it has to live in the hitbox layer — which is already detached from ability lifetime and already aged centrally by `SpellResolver.Tick`. F's drag does use the locked-ability pattern, because F locks the caster.
